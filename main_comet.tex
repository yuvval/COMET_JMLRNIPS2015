...
\begin{algorithmic}[1]
   ...
   \STATE Compute the Cholesky decomposition of $\A$ using $\cholL$ \cite{Davis05rowchol}.
...
The Cholesky decomposition of $\newW$ and of $\A$ can be evaluated efficiently in $O(d^2)$ using row modification \cite{Davis05rowchol}. $(\newW)^{-1}$ can also be evaluated in $O(d^2)$, given $\W^{-1}$, using the Woodbury inverse matrix identity \cite{woodbury1950inverting}, see Appendix B for details.
...
\begin{algorithmic}[1]
   ...
   \IF {Update equals its previous iteration}
...
To compute efficiently the coefficients of \eqref{PDUpdateCondQuadForm} we maintain $\cholL$, the Cholesky decomposition of $\W$, and update it following a block coordinate step. Moreover, we derive $\cholL_A$ A Cholesky decomposition of $A$ from $\cholL$. Both terms take $O(d^2)$ to compute \cite{Davis05rowchol}.
