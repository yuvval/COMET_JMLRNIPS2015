...
In practice for evaluating the condition in \eqref{PDUpdateCondQuadForm} efficiently, we maintains an updated Cholesky root matrix $\cholL$ s.t. $\W = \cholL\T \cholL$. $\cholL$ enables to efficiently derive a Cholseky for $\A$ \citep{Davis05rowchol} and in turn evaluate efficiently the terms of \eqref{PDUpdateCondQuadForm} with a cost of $O(d^2)$. 
