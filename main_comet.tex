...
In addition, while the in theory the $\log \det$ term ensures that the optimum is within the PD cone, in practice the intermediate iterates for 1-st order methods are not necessarily confined to the cone. The reason is that the gradients of the $\log \det$ term are not Lipschitz \footnote{one might add a small constant times the identity to the model, but then the step size will have to be very small to ensure remaining within the cone, resulting in slower convergence.}, and any given step size might take the update outside the cone. 
...
We propose an algorithm that alleviates these problem by using efficient block-coordinate descent that keeps optimization within the boundaries of the PD cone while essentially amortizing the cost of matrix inversion.
In general, block-coordinate descent also enjoys provably fast convergence rates, and is especially useful when the block update can be performed efficiently, as we show below. We derive a method that enables using as large a step size as possible and still remain within the PD cone for all iterates, based on the Schur complement condition for PD matrices \citep[p. 650]{boyd2004convex}.
...
As discussed above, taking a coordinate step may take $\newW$ out of the PD cone, even if the objective holds a $\log\det$ barrier term. We show that given $\W$ and $\mat{G}$ the step size can be bounded to guarantee that $\newW$ remains PD, and that the bound can be computed efficiently using the Schur complement condition for positive definiteness.


{\bf ============ beginning of section to move to an appendix. KEEP ONLY THE CONCLUDING PARAGRAPH. EQUATION 4 (W=W+G, should also be inlined since it is trivial. } 
...
{\bf ============ end of section to move to an appendix }
...
\begin{theorem}
Let $\W^t$ be the $t$-th iterate of Algorithm \ref{alg:comet} with objective function $\tL$, sampling each column-row $i$ with probability $\frac{1}{d}$. Let $\tilde{L}^*$ be the optimal value of $\tL$ on the PD cone. Let $\beta^* \geq \beta$ be the strong convexity parameter of $\tL$, $M^1$ a constant depending on the norm of the dataset, $1 > \rho >0, \epsilon>0$. Then:
...
\begin{algorithm}[t]
   ...
   \label{alg:spcomet}
\begin{algorithmic}[1]
   \STATE {\bfseries input:} training data, $\alpha$, $\beta$, $\lambda$
   \STATE {\bfseries initialize:} 
...
Given the decomposition $\Vg$, we can optimize \eqref{sparse_group_loss} using standard block-coordinate methods for non-smooth objectives. Specifically we use the method proposed by \citet{richtarik2014iteration}.
...
\acks{We would like to thank Prof. Tim Davis for fruitful discussions and for his assistance with utilizing the Cholesky decomposition row-column updates with the CHOLMOD package.} 
