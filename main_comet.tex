...
\begin{algorithmic}[1]
   ...
   \REPEAT 
   \STATE Draw a coordinate $k \in \{1 \ldots d\}$ uniformly at random. %{\bf XXX FIX XXX}
...
\end{subfigure}%
\setcounter{subfigure}{0} % reset subfigure counter for the next figure (due to a bug in the subfigure package)
\setcounter{figure}{3} % subfigure package has a bug (it counts the subfigure as figure). a hack is to reset figure counter
}
\vskip -10pt
\end{figure}
...
\noindent {\bf Acknowledgements} We thank Prof. Tim Davis for insightful discussions and assistance with utilizing the Cholesky decomposition row-column updates.
