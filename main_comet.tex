...
%\todo{Related work on sparse metrics }
...
In addition, while in theory the $\log \det$ term ensures that the optimum is within the PD cone, in practice the intermediate iterates for first order methods are not necessarily confined to the cone. The reason is that the gradients of the $\log \det$ term are not Lipschitz \footnote{One can force the objecitve to be Lipschitz by adding a small constant times the identity matrix to the model $\W$. However, even in that case the step size will have to be small to ensure remaining within the cone, resulting in slower convergence.}, and any given step size might take the update outside the cone. 
...
The PD metric learned above can be used for extracting new features, but it does use the structure often found in feature interactions. Moreover, inference involves multiplication by a dense $d \times d$ matrix. In most high-dimensional learning problems, many of the features do not interact intensely with each other, meaning that their corresponding off-diagonal terms are $0$. We suggest learning a metric such that only a small set of features can to interact with \emph{any} of the other features. Any two features which are not in the ``interacting set'' will have their interaction term set to $0$. Note that a feature from the ``interacting set'' can still have an interaction term with a feature from the ``non-interacting set''. We also maintain an independent weight for each feature, corresponding to the diagonal term in the learned similarity matrix. Note that the interaction structure we suggest leads to sparse matrices, allowing for faster inference both during training and test time. To see an example of a matrix with this interaction structure, see Figure \ref{fig:spmatrix}. 
...
\textbf{BoostMetric} \citep{boost}. Based on the observation that any positive semidefinite matrix can be decomposed into linear positive combination of rank-1 matrices, BoostMetric uses rank-1 PSD matrices as weak learners within a boosting based learning process.
...
\floatsetup[figure]{capposition=beside,capbesideposition={right}}
\begin{figure}[ht]
\centering
...
\captionsetup{font=small}
    ...
    \begin{subfigure}[]%{0.5\textwidth}
        \centering
%        \includegraphics[height=1.2in]{sparse_W_visualization}
        \includegraphics[trim=+1.6cm 0 0 0, width=4cm]{sparse_W_visualization}
        \label{fig:spmatrix}
        %\caption{Lorem ipsum}
    \end{subfigure}%
    ~ 
    \begin{subfigure}[]%{0.5\textwidth}
        \centering
        \includegraphics[ width=4cm]{V_features_vs_infogain}
        %\caption{Lorem ipsum, lorem ipsum,Lorem ipsum, lorem ipsum,Lorem ipsum}
    \end{subfigure}
    \caption{The interpretability of the sparse matrix: (a) shows the absolute values of the elements of $\W$, trained on RCV1, where features were sorted according to their \textit{infogain} (see text). (b) denotes the norm of each of the coordinate features against its i. In both (a) and (b) we see that sparse COMET learns the informative features and mostly zeros the non informative ones.  }
\end{figure}
...
\acks{We would like to thank Prof. Tim Davis for fruitful discussions and for his assistance with utilizing the Cholesky decomposition row-column updates with the CHOLMOD solver.} 
